...
| 短按 **OK** | 编辑或确认参数、切换 SCD30 设置、开始/停止记录，或确认对话框 |
| 短按 **BACK** | 退出记录参数编辑、从记录页返回仪表页、打开或取消清空历史对话框，或取消退出对话框 |
| 长按 **左 / OK / BACK** | 在记录页按住左键 2 秒进入 SCD30 校准；OK 在手动校准页确认校准，其他页面切换背光常亮；BACK 打开退出确认 |

## 数据记录

在仪表页按 **左** 打开 **Data Recording**。用 **上 / 下** 选择字段，短按
**OK** 进入编辑，再用 **左 / 右** 切换选项。选中 **Start** 后按 **OK** 开始
记录；再次按 **OK** 停止。两个设置项均可向左或向右循环切换。编辑参数时短按
**BACK** 退出编辑；在记录页短按 **BACK** 返回仪表页。
...
在 **Data Recording** 记录页按住 **左** 2 秒进入 **SCD30 Calibration**。
用 **上 / 下** 移动光标，短按 **OK** 切换气压计补偿或自动环境校准（ASC）。
两个设置保存在 `/ext/co2_sniffer/settings.bin`，应用启动时会先加载再初始化
SCD30。配置文件不存在、为空或无效时，默认开启气压计补偿并关闭 ASC。

选择 **Manual calibration** 并阅读提示。进入校准输入页后，顶部正计时显示进入
校准状态的时间；**左 / 右** 循环选择四位数字，**上 / 下** 循环修改当前数字。
SCD30 强制校准的有效范围是 `0400–2000 ppm`。短按 **OK** 检查校准值，再按住
**OK** 2 秒，进度条完成后才会执行校准。确认前应让传感器在稳定参考空气中至少
运行 2 分钟，整个校准过程中保持面部远离传感器，并使用已知参考浓度，不要假定
室外空气恰好是 400 ppm。启动及每次传感器重连后的前 10 秒读数不会写入历史曲线。
...
`SW/` 是 Flipper 应用（FAP，appid `co2_sniffer`，分类 DIY，v0.5）。工作线程的
主要流程如下：
